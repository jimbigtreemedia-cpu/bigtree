...
const iconMap = {
    camera: _jsx(Camera, { size: 28 }),
    shirt: _jsx(Shirt, { size: 28 }),
    sparkles: _jsx(Sparkles, { size: 28 }),
    video: _jsx(Video, { size: 28 }),
    workflow: _jsx(Workflow, { size: 28 }),
    diamond: _jsx(Gem, { size: 28 }),
    palette: _jsx(Palette, { size: 28 }),
    "user-round": _jsx(UserRound, { size: 28 }),
    megaphone: _jsx(Megaphone, { size: 28 })
};
const sectionCopy = {
    clothing: {
...
const ServiceCard = ({ service }) => {
    ...
    return jsxs(Link, { to: `/services/${service.id}`, "data-analytics-cta": `services_card_${service.id}`, "data-analytics-section": "services_grid", className: "group p-8 rounded-3xl bg-white dark:bg-slate-900 premium-border dark:border dark:border-slate-800 soft-shadow hover:-translate-y-1 transition-all duration-300 flex flex-col", children: [
            jsxs("div", { className: "relative w-full mb-8 rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 aspect-square", children: [
                    _jsx(CoverMedia, { src: service.image || "https://picsum.photos/400/400", alt: service.title, parentClassName: "absolute inset-0 w-full h-full", className: "w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" }),
