...
const Legal = ({ type }) => {
    ...
    return jsx("section", { className: "py-32 bg-background-base dark:bg-slate-950 border-t border-black/5 dark:border-white/5", children: jsxs("div", { className: "max-w-4xl mx-auto px-6 lg:px-8", children: [
                ...
                jsxs("div", { className: "mt-8 flex flex-col sm:flex-row gap-4", children: [
                        _jsx(Link, { to: "/contact", className: "flex-1 h-12 flex items-center justify-center rounded-xl bg-primary text-white font-bold hover:bg-slate-800 dark:hover:bg-accent-teal transition-all shadow-lg shadow-primary/20", children: "Contact Us" }),
                        _jsx(Link, { to: "/", className: "flex-1 h-12 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all", children: "Back Home" })
                    ] })
            ] }) });
};
