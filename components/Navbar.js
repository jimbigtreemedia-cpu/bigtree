    ...
    return jsxs("nav", { className: `fixed top-0 left-0 right-0 z-50 border-b ${prefersReducedMotion ? "transition-none" : "transition-all duration-300"} ${isScrolled || mobileMenuOpen ? "bg-white/90 dark:bg-slate-950/90 backdrop-blur-md shadow-sm border-slate-200 dark:border-slate-800" : "bg-transparent border-transparent"}`, children: [
            jsx("div", { className: "mx-auto max-w-7xl px-6 lg:px-8", children: jsxs("div", { className: "flex h-20 items-center justify-between gap-2", children: [
                        ...
                        jsxs("div", { className: "hidden md:flex items-center gap-4", children: [
                                jsx("button", { onClick: toggleTheme, className: "p-2 rounded-full text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors", "aria-label": "Toggle Theme", children: theme === "light" ? _jsx(Moon, { size: 20 }) : _jsx(Sun, { size: 20 }) }),
                                _jsx(Link, { to: "/free-trial", className: "free-trial-glow inline-flex h-10 items-center justify-center rounded-full border border-primary/20 bg-primary/10 px-4 text-sm font-bold text-primary transition-colors hover:bg-primary/15 dark:border-accent-teal/30 dark:bg-accent-teal/10 dark:text-accent-teal dark:hover:bg-accent-teal/20", children: "Free Trial" }),
                                _jsx(Link, { to: "/contact", className: "flex h-11 items-center justify-center rounded-full bg-primary px-8 text-sm font-bold text-white shadow-lg shadow-primary/10 transition-all hover:bg-slate-800 hover:shadow-primary/20 hover:scale-105 active:scale-95 dark:hover:bg-accent-teal dark:text-white", children: "Get Started" })
                            ] }),
                        jsxs("div", { className: "ml-1 flex shrink-0 items-center gap-1.5 md:hidden sm:ml-2 sm:gap-3", children: [
                                _jsx(Link, { to: "/free-trial", className: "free-trial-glow whitespace-nowrap rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1.5 text-[11px] font-bold text-primary dark:border-accent-teal/30 dark:bg-accent-teal/10 dark:text-accent-teal sm:px-4 sm:py-2 sm:text-xs", children: "Free Trial" }),
                                jsx("button", { onClick: toggleTheme, className: "p-2 rounded-full text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors", "aria-label": "Toggle Theme", children: theme === "light" ? _jsx(Moon, { size: 20 }) : _jsx(Sun, { size: 20 }) }),
                                jsx("button", { className: "flex items-center justify-center p-2 text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 rounded-lg transition-colors", onClick: () => setMobileMenuOpen(!mobileMenuOpen), "aria-label": mobileMenuOpen ? "Close menu" : "Open menu", "aria-expanded": mobileMenuOpen, children: mobileMenuOpen ? _jsx(X, { size: 24 }) : _jsx(Menu, { size: 24 }) })
                            ] })
                    ] }) }),
